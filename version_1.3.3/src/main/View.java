...
public class View {
	...
	private void drawVerletStats() {
		...
		p5.text("Springs: " + App.PSYS.getPhysics().springs.size(), 3, 10);
		p5.text("Particles: " + App.PSYS.getPhysics().particles.size(), 3, 20);
		p5.text("Spring Str : " + App.DF3.format(App.SPR_STR), 3, 50);
		p5.text("Drag : " + App.DF3.format(App.PSYS.getPhysics().getDrag()), 3, 30);
...
//		p5.translate(1600, 150);
	...
	private void drawVerletSprings() {
		...
		for (VerletSpring2D s : App.PSYS.getSprings()) { p5.line(s.a.x, s.a.y, s.b.x, s.b.y); }
		p5.noStroke();
	}
	private void drawNodes() {
		for (FSys.Node n : App.FSYS.nodes) {
			p5.stroke(n.getColor(), 100, 100);
			p5.ellipse(n.getPos().x, n.getPos().y, n.radius * App.SCALE, n.radius * App.SCALE);
			p5.stroke(Color.NODE_F);
			p5.ellipse(n.getPos().x, n.getPos().y, n.radius, n.radius);
			p5.noStroke();
		} if (Edit.selectedNode != null) {
			p5.stroke(Color.RED);
			p5.ellipse(Edit.selectedNode.getPos().x, Edit.selectedNode.getPos().y, 30, 30);
		}
	}
