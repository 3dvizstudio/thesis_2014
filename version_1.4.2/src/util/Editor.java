...
public class Editor {
	...
	public void marshal(ArrayList<FSys.Node> nodes, ArrayList<FSys.Relation> relations) {
	...
	public void debug() {
		for (FSys.Node n : map.keySet()) {
			System.out.println(n.name + map.get(n));
		} for (FSys.Relation r : relations) {
			System.out.println(nodes.get(r.from).name + "." + r.from + ":" + nodes.get(r.to).name + "." + r.to);
