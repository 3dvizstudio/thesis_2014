...
/*		p5.stroke(0xffffff00);
	...
	/*	p5.pushMatrix();
		p5.translate(1200, 50);
		p5.fill(Color.PHYS_TXT);
		p5.text("Springs: " + App.PSYS.getPhysics().springs.size(), 0, 0);
		p5.text("Particles: " + App.PSYS.getPhysics().particles.size(), 0, 10);
		p5.text("Behaviors: " + App.PSYS.getPhysics().behaviors.size(), 0, 20);
		p5.text("Attractors: " + App.PSYS.getAttractors().size(), 0, 30);
		p5.text("Spring Str : " + App.DF3.format(App.SPR_STR), 0, 40);
		p5.text("Drag : " + App.DF3.format(App.PSYS.getPhysics().getDrag()), 0, 50);
		p5.text("Separation : " + App.DF3.format(App.ATTR_RAD), 0, 60);
		p5.noFill();
		p5.popMatrix();*/
	}
